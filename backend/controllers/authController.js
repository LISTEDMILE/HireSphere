    ...
    async (req, res, next) => {
        ...
        if (userType == "recruiter") {
            try {
                user = await UserRecruiter.findOne({
                    ...
                    "_id username password userType aboutRecruiter.profilePicture",
                );
            ...
            try {
                user = await UserEmployee.findOne({
                    ...
                    "_id username password userType aboutEmployee.profilePicture",
                );
        ...
        bcrypt
            ...
            .then((isMatch) => {
                ...
                if (
                    userObj.aboutEmployee &&
                    userObj.aboutEmployee.profilePicture
                ) {
                    ProfileUrl = userObj.aboutEmployee.profilePicture;
                    delete userObj.aboutEmployee;
                }
                if (
                    userObj.aboutRecruiter &&
                    userObj.aboutRecruiter.profilePicture
                ) {
                    ProfileUrl = userObj.aboutRecruiter.profilePicture;
                    delete userObj.aboutRecruiter;
                }
    ...
    if (req.session.isLoggedIn && req.session.user) {
        ...
        if (req.session.user.userType == "recruiter") {
            try {
                ...
                ).select("aboutRecruiter.profilePicture");
                ...
                if (user.aboutRecruiter.profilePicture)
                    ProfileUrl = user.aboutRecruiter.profilePicture;
            } catch (err) {
                console.error("Error finding user:", err);
                res.status(500).json({
                    isLoggedIn: false,
            ...
            try {
                const user = await UserEmployee.findById(
                    req.session.user._id,
                ).select("aboutEmployee.profilePicture");
                ...
                if (user.aboutEmployee.profilePicture)
                    ProfileUrl = user.aboutEmployee.profilePicture;
            } catch (err) {
                console.error("Error finding user:", err);
                res.status(500).json({
                    isLoggedIn: false,
        ...
        res.json({
            isLoggedIn: true,
            user: { ...req.session.user, profilePicture: ProfileUrl },
        });
    }
};
...
exports.postDeleteAccount = async (req, res, next) => {
    try {
        ...
        if (req.session.user.userType === "recruiter") {
            ...
            }).select("_id password aboutRecruiter.profilePicture");
        } else {
            user = await UserEmployee.findOne({
                username: req.session.user.username,
            }).select("_id password aboutEmployee.profilePicture");
        }
        ...
        if (req.session.user.userType === "recruiter") {
            ...
            if (
                ...
                try {
                    ...
                    console.error("Error deleting Profle Picture", err);
                    res.status(500).json({
                        error: "Failed to delete Profile Picture",
                    });
                }
